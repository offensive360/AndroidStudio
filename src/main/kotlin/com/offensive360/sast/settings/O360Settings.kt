...
class O360Settings : PersistentStateComponent<O360Settings.State> {
    ...
    data class State(
        ...
        var scanDependencies: Boolean = false,
        var scanLicenses: Boolean = false,
        var scanMalware: Boolean = false
    )
    ...
    var scanDependencies: Boolean
        get() = state.scanDependencies
        set(value) { state.scanDependencies = value }

    var scanLicenses: Boolean
        get() = state.scanLicenses
        set(value) { state.scanLicenses = value }

    var scanMalware: Boolean
        get() = state.scanMalware
        set(value) { state.scanMalware = value }
